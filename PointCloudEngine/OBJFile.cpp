...
bool OBJFile::LoadOBJFile(std::wstring filename, OBJContainer* container)
{
    try
    {
        ...
        WARNING_MESSAGE_ON_FAIL(objFile.is_open(), L"Cannot open OBJ file \"" + filename + L"\"!");
    ...
    bool isValid = !(container->buffers.positions.empty() || container->buffers.normals.empty() || container->buffers.textureCoordinates.empty() || container->meshes.empty());
    WARNING_MESSAGE_ON_FAIL(isValid, L"Mesh in OBJ file \"" + filename + L"\" is not valid!\nMake sure that the file contains positions \"v\", texture coordinates \"vt\", normals \"vn\", faces \"f\" and materials \"usemtl\" with diffuse textures \"map_Kd\"!");
