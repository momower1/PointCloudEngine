...
{
	HDF5File(std::string(filename.begin(), filename.end()));
}
...
void HDF5File::SaveTexture2DToDataset(std::string name, ID3D11Texture2D* texture)
{
	...
	std::vector<USHORT> buffer(subresource.DepthPitch / 2);
	memcpy(buffer.data(), subresource.pData, subresource.DepthPitch);
	...
	// Create the dataset
	hid_t datasetID = H5Dcreate(fileID, name.c_str(), H5T_NATIVE_USHORT, dataspaceID, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
